...
def convert_unicode_to_katex(text):
    """Replace Unicode math symbols — ONLY inside math mode $...$ or $$...$$.""" 
    def _replace_math_unicode(s):
    ...
    while i < len(text):
        if text[i:i+2] == "$$" and (i == 0 or text[i-1] != "\\"):
            ...
            if end == -1:
                out.append(text[i:])
                break
            out.append("$$" + _replace_math_unicode(text[i+2:end]) + "$$")
            i = end + 2
            ...
            if end == -1:
                out.append(text[i:])
                break
            out.append("$" + _replace_math_unicode(text[i+1:end]) + "$")
            i = end + 1
        else:
            out.append(text[i])
            i += 1
    result = "".join(out)
    # Handle Unicode math chars OUTSIDE math mode: wrap in $...$
    unicode_math_map = {
        "≥": r"$\ge$", "≤": r"$\le$", "≠": r"$\ne$", "≈": r"$\approx$",
        "∈": r"$\in$", "∉": r"$\notin$", "ℝ": r"$\mathbb{R}$",
        "∆": r"$\Delta$", "±": r"$\pm$", "∞": r"$\infty$",
        "→": r"$\rightarrow$", "⇒": r"$\Rightarrow$", "⇔": r"$\Leftrightarrow$",
    }
    for ch, repl in unicode_math_map.items():
...
def split_mcq_options(q_text):
    ...
    # Fallback: inline pattern (A. ... B. ... C. ... D. ... on same line)
    inline_pattern = re.compile(r"(?:^|\s{2,}|\t)([A-D])[.\):\s]\s*")
    matches = list(inline_pattern.finditer(q_text))
    if len(matches) >= 4:
        matches = matches[-4:]  # Take last 4 matches (A, B, C, D)
        stem = q_text[:matches[0].start()].strip()
        options = []
        for i, m in enumerate(matches):
            start = m.end()
    ...
    for idx, (q_num, m) in enumerate(unique_matches):
        ...
        if lg_match:
            ...
            if ans_m:
                ...
                # No explanation header found — treat entire block as question
                q_text_raw = block
                exp_text_raw = ""
