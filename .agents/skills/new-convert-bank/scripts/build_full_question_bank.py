...
def run_phase1_extract_and_crop(
    ...
    print(f"🤖 [Pha 1.2] Đang chạy Local DocLayout-YOLO bóc tách hình vẽ & Bảng Biến Thiên...")
    try:
        import torch
        ...
        for page_idx, img_path in enumerate(image_files):
            ...
            for r in results:
                ...
                for box in r.boxes:
                    ...
                    bx_w = x2 - x1
                    bx_h = y2 - y1
                    ar = bx_w / max(1.0, bx_h)

                    # Quy tắc 1: Hình vẽ / Đồ thị thực sự (figure)
                    if cls_name == "figure":
                        if conf >= 0.18 and bx_w >= 80 and bx_h >= 80 and bx_h <= 0.65 * img_h:
                    ...
                    # Quy tắc 2: Bảng biểu / Bảng biến thiên (table)
                        ...
                        if conf >= 0.20 and bx_w >= 250 and bx_h >= 100 and bx_h <= 0.65 * img_h:
                            page_candidates.append({
                                "cls": "bbt",
                                "conf": conf,
                                "box": [x1, y1, x2, y2],
                    ...
                    # Quy tắc 3: Bảng biến thiên nằm dưới tiêu đề "Bảng biến thiên"
                        ...
                        for bbt in bbt_rects:
                            if y1 >= bbt["y1"] - 50 and y1 <= bbt["y2"] + 250:
                                if bx_w >= 600 and bx_h >= 180 and ar >= 1.2:
                                    page_candidates.append({
                                        "cls": "bbt",
                                        "conf": conf,
                                        "box": [x1, y1, x2, y2],
            ...
            clean_boxes = sorted(clean_boxes, key=lambda x: x['box'][1])
        ...
        print(f"✅ Đã cắt tự động {total_cropped} hình vẽ & BBT chuẩn xác vào: {figures_dir}")
...
def parse_pdf_to_questions(pdf_path, manifest, default_topic, default_grade, forced_type, review):
    ...
    for idx, (q_num, m) in enumerate(unique):
        ...
        # Map hình theo tọa độ: dùng page_question_offsets
        q_figures = []
        q_pnum, q_y = get_question_y_in_page(pymupdf.open(pdf_path), q_num, page_question_offsets, upto_page=sol_start_page)
                ...
                if p_name in manifest_by_page:
                    for fig in manifest_by_page[p_name]:
                        if fig["filename"] not in [f["filename"] for f in q_figures]:
                            # hình nằm ngay sau câu N trên cùng trang → thuộc câu N
                            q_figures.append(fig)
        else:
            # fallback: lấy theo pages_in_block
            for p_name in pages_in_block:
                if p_name in manifest_by_page:
                    for fig in manifest_by_page[p_name]:
                        if fig["filename"] not in [f["filename"] for f in q_figures]:
        ...
        # Nếu ảnh chính không có nhưng có explanation ảnh → đẩy thành imageFileName
