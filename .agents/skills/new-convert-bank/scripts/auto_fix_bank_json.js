...
function fixMathExpression(expr) {
  ...
  // 6. Sửa lỗi \left( ... \frac{a}{b mà thiếu dấu đóng ngoặc tròn
  ...
  if (vnRegex.test(s) && !s.includes('\\text{')) {
    s = s.replace(/([a-zA-ZàáạảãâầấậẩẫăằắặẳẵèéẹẻẽêềếệểễìíịỉĩòóọỏõôồốộổỗơờớợởỡùúụủũưừứựửữỳýỵỷỹđĐ\s]{2,})/g, (m) => {
      if (vnRegex.test(m) && !m.includes('\\') && !m.includes('{') && !m.includes('}')) {
        return ' \\text{' + m.trim() + '} ';
      }
      return m;
    });
...
function autoFixText(text) {
  ...
  // 5. Cân bằng dấu dollar $ nếu bị lẻ
