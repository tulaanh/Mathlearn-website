...
type Props = {
  ...
  setAttachedTestId?: (v: string) => void;
...
export default function DocumentEditorFields(p: Props) {
  ...
  return <>
    ...
    {p.documentType === "normal" && (
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900"><h2 className="font-bold dark:text-white">📝 Bài kiểm tra đính kèm (tùy chọn)</h2><p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Chọn một bài kiểm tra đã xuất bản để hiển thị nút làm bài ngay bên dưới nội dung tài liệu. Có thể bỏ trống nếu tài liệu không cần bài kiểm tra.</p><label className="mt-4 block text-sm font-semibold dark:text-slate-200">Bài kiểm tra<select value={p.attachedTestId ?? ""} onChange={(e) => p.setAttachedTestId?.(e.target.value)} className="mt-2 h-11 w-full rounded-xl border px-3 dark:border-slate-700 dark:bg-slate-950 dark:text-white"><option value="">— Không đính kèm —</option>{(p.testOptions ?? []).map((t) => <option key={t.id} value={t.id}>{t.title} ({t.grade})</option>)}</select></label></section>
    )}
